"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const joi_1 = __importDefault(require("@hapi/joi"));
const validator_1 = require("../../helpers/utils/validator");
exports.default = {
    param: joi_1.default.object().keys({
        id: (0, validator_1.JoiObjectId)().required(),
    }),
    create: joi_1.default.object().keys({
        name: joi_1.default.string().trim().min(2).max(100).required(),
        permissions: joi_1.default.array().items((0, validator_1.JoiObjectId)()).unique().min(1).required(),
    }),
    update: joi_1.default.object().keys({
        name: joi_1.default.string().trim().min(2).max(100),
        permissions: joi_1.default.array().items((0, validator_1.JoiObjectId)()).unique().min(1),
    }),
};
//# sourceMappingURL=schema.js.map